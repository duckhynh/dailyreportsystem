...
flowchart TD
    ...
    subgraph App [Daily Report Application]
...
```mermaid
flowchart TD
    subgraph Frontend [Nhánh Client]
        UI[React/Vue Web SPA]
        Mobile[React Native Mobile App]
    end
    
    subgraph Backend [Nhánh Server - .NET Core]
        API_Layer[WebApi Package <br> Controllers, Middlewares]
        App_Layer[Application Package <br> Services, DTOs]
        Infra_Layer[Infrastructure Package <br> Repositories, EF Core]
        Core_Layer[Core Package <br> Entities, Interfaces]
    end
...
flowchart TD
    ...
    subgraph Cloud [Azure / AWS]
        WAF{"Web Application Firewall <br> Mạng lưới DMZ"}
        Backend[Backend API Server <br> Ubuntu + Kestrel]
        DBNode[(SQL Server Database <br> Private Subnet)]
    end
